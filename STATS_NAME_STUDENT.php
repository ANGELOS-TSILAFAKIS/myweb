...
if (!$_POST)   {
   // φορμα επιλογης
   $block = "<h1>ΛΙΣΤΑ ΦΟΙΤΗΤΩΝ</h1>";
    
 /*

		 $basic_student_registration = "INSERT INTO basic_student_registration(student_registration_id,subject_practice_id,
		                          surname, name, name_father, name_mother, Registration_Number,  
								  half_year_insertion, Identity_Card,  afm,   TAX_OFFICE,                                         
								  date_added, date_modified)
								  VALUES('".$student_registration_id."','".$subject_practice_id."',
								 '".$_POST['surname']."','".$_POST['name']."','".$_POST['name_father']."',
								 '".$_POST['name_mother']."','".$_POST['Registration_Number']."',
								 '".$_POST['half_year_insertion']."','".$_POST['Identity_Card']."', 
								 '".$_POST['afm']."','".$_POST['TAX_OFFICE']."', 
								 now(), now())";
...
*/        
   ...
   //ανακτηση δεδομενων
   $get_LIST_students_sql = " SELECT request_master_id,
                        concat_ws(' ', surname, name) AS name_surname 
					    FROM basic_student_registration
					    ORDER BY surname, name ";
   
   $get_LIST_students_result = mysqli_query($mysqli,  $get_LIST_students_sql)
		                    or die(mysqli_error($mysqli));
							
	if (mysqli_num_rows($get_LIST_students_result) < 1) {
        // δεν υπαρχουν εγγραφες
		$block .= '<h3>δεν υπαρχουν εγγραφες</strong></h3>';
	
	} else {
		$block .= '
		<form method="POST" action="STATS_NAME_STUDENT.php"  >
		<p><strong>ΕΠΕΛΕΞΕ ΤΟΝ ΦΟΙΤΗΤΗ ΠΟΥ ΘΕΛΕΙΣ ΝΑ ΔΕΙΣ </strong></p><br/> 
		<select name="select_student_registration_id">
		    <option value="-1" > -- επελεξε φοιτητη -- </option> "';
			
			while ($info_table = mysqli_fetch_array($get_LIST_students_result))  {
			    $request_master_id = $info_table["request_master_id"];
				$name_surname = stripslashes($info_table["name_surname"]);
				
				$block .=  "<option value=".$request_master_id.">".$name_surname."</option>";
				
			}
			
			$block .= '"
			</select>
			<p><input type="submit" name="submit" value="δες στοιχεια" ></p>
			<form> ';
	}	
			
	mysqli_free_result( $get_LIST_students_result);		
								
} else if ($_POST)  {

    //ελεγχος για το select
	if ($_POST["select_student_registration_id"] == "" ) {
	   header("Location: STATS_NAME_STUDENT.php");
	   exit();
	}
	...
	$get_basic_student_registration = " SELECT 
					    ...
					    FROM basic_student_registration
					    WHERE request_master_id = '".$_POST['select_student_registration_id']."' ";
   ...
   */
   ...
   $get_When_contacting_2_result = mysqli_query($mysqli, $get_When_contacting_2)
	...
	$block .= '<br/>
	        <p align="center">
			<a href="STATS_NAME_STUDENT.php"><p>επελεξε αλλον φοιτητη</p></a>  '  ;
